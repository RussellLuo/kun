...
func Example_newV2() {
	...
	r.MustRegister(httpapp.NewV2("hi", new(Hi)).MountOn("greeter", "/hi").App)

	// Typically located in `func init()` of package bye.
	r.MustRegister(httpapp.NewV2("bye", new(Bye)).MountOn("greeter", "/bye").App)

	// Typically located in `func init()` of package greeter.
	r.MustRegister(httpapp.NewV2("greeter", new(Greeter)).App)
